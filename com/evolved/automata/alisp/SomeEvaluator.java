...
public class SomeEvaluator implements CompiledEvaluator, RovingLexicalEvaluator {
	...
	public SomeEvaluator(Environment env)
	{
		this.env = env;
	}
			...
			switch (state)
			{
				...
				default:
					...
					if (!Environment.isNull(output))
						return resetReturn(iterationListArg[argumentIndex]);
					state=BINDING_VARIABLE;
					argumentIndex++;
			}
		}
	...
	{
		SomeEvaluator ceval = new SomeEvaluator(env);
