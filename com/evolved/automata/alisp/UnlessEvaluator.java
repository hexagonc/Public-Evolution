...
public class UnlessEvaluator implements CompiledEvaluator{
	...
	public UnlessEvaluator(Environment env)
	{
		this.env = env;
	}
	...
	public Argument eval(Argument[] args) {
		if (args == null)
			return null;
		conditionValue=getFinalValue(resume, args[0]);
		...
		{
			Argument output = getFinalValue(resumeChild, args[1]);
	...
	{
		UnlessEvaluator ceval = new UnlessEvaluator(env);
	...
	protected Argument evaluateArgument(boolean r, Argument arg)
	{
		CompiledEvaluator com = arg.getEvaluator();
		return com.getCompiledResult(r);
