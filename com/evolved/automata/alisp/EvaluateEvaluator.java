...
public class EvaluateEvaluator implements CompiledEvaluator {
	...
	public EvaluateEvaluator(Environment env)
	{
		this.env = env;
	}

	// Evaluates a string representation of an s-expression programmatically.
	// This function should not be used with string code that returns continuations
	@Override
	public Argument eval(Argument[] args) {
		if ((args == null)||(args.length!=1))
			return null;
		
		Argument expression = getFinalValue(resume, args[0]);
	...
	{
		EvaluateEvaluator ceval = new EvaluateEvaluator(env);
		...
		for (int j=0;j<sargs.length;j++)
		{
			...
			else
			{
				if (nValue!=null&&nValue.isCons()&&nValue.innerList.length>0&&nValue.innerList[0].isIdentifier())
				{
					...
					c = env.getFunction((String)nValue.innerList[0].oValue).clone();
					if (c!=null)
					{
